...
class GoogleTranslator {
    ...
    async getTranslator(text) {

        const sourceLanguage = this.sourceLanguage.value === 'auto' ? await this.detectLanguage(text) : this.sourceLanguage.value
        const targetLanguage = this.targetLanguage.value

        if (sourceLanguage === targetLanguage) return text

        // verificar si realmente tenemos disponibilidad de la traduccion
        try {
            ...
            if (status === 'unavailable') {
                throw new Error(`Translator de ${sourceLanguage} a ${targetLanguage} no disponible`)
            }
        ...
        try {
            if (
                !this.currentTranslator || this.currentTranslatorKey !== translatorkey
            ) {
                this.currentTranslator = await window.Translator.create({
                    ...
                    monitor: (monitor) => {
                        monitor.addEventListener("downloadprogress", (e) => {
                            this.outputText.innerHTML = `<span class="loanding">Descargando modelo:
    ...
    async translate() {
        ...
        try {
            const translation = await this.getTranslator(text)
            ...
            if (!hasSupport) {
                this.outputText.textContent = '¡Error! no Tienes suporte nativo a la API de traduccion con IA'
                return
            }
    ...
    async swapLanguages() {
        ...
        // primero dectetar si es auto  para saber que idioma se va a traducir
    ...
    async startVoiceRecognition() {
        const hasNativeReconognitionSupport = "SpeechRecognition" in window || "webkitSpeechRecognition" in window
        if (!hasNativeReconognitionSupport) return
        ...
        const language = this.sourceLanguage === 'auto'
    ...
    async speakTranslation() {
        ...
        uttarence.lang = this.getFullLanguageCode(this.targetLanguage.value)
        uttarence.rate = 0.8
        ...
        window.speechSynthesis.speak(uttarence)
    }
        ...
        try {
            ...
            if (!this.currentDetector) {
                this.currentDetector = await window.LanguageDetector.create({
                    expectedInputLanguage: GoogleTranslator.SUPPORTED_LANGUAGES
                })
            }
            ...
            console.error("No se ha podido averiguar el idioma", error)
